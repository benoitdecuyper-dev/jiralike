...
export default function Search({ projetId, onOpen }) {
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      listTickets(projetId, filters)
        .then(r => { setRows(r); setErr(''); })
        .catch(e => { console.error(e); setErr('Recherche impossible : ' + e.message); });
    }, 200); // debounce
  ...
  return (
    <>
      ...
      <div className="searchbar">
        <span>⌕</span>
        <input placeholder="Rechercher dans les titres et descriptions…" value={q} onChange={e => setQ(e.target.value)} />
      </div>
